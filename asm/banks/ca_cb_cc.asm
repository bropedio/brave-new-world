...
; ------------------------------------------------------------------------
; Switch back to Sabin/Shadow at Imperial Camp (A)
;
; Modified as part of `castle-party.asm`
; We no longer "assign character to party 1" or "remove from party"
; We now explicitly switch parties instead.

org $CB0BC4
  db $C0,$F3,$02,$CC,$0B,$01 ; BRL target $CB0BCC
...
; ------------------------------------------------------------------------
; Switch back to Sabin/Shadow in Imperial Camp (B)
; Part of `castle-party.asm` patch
;
; NOTE: The changes at top here are artifacts from a bad engine,
...
org $CB1317
  db $02,$88                 ; Begin Cyan:02 action queue, 8 bytes long, wait
  db $C3                     ; - Set speed to fast
  db $94                     ; - Move up 6 tiles
  db $81,$81,$81             ; - Move right 3 tiles ; TODO: Why not $89
  db $80,$80                 ; - Move up 2 tiles    ; TODO: Why not $84
  db $FF                     ; - END
  db $39                     ; Free screen
...
; ------------------------------------------------------------------------
; Recruiting Cyan (during Imperial Camp Doma scenes)
; Modified by `castle-party.asm`
...
org $CB9AAE
  ...
  db $3F,$02,$02              ; Cyan:02 -> Party 2 (instead of 1)
...
; ------------------------------------------------------------------------
; Transition to "Doma Being Poisoned" scene
; Modified by `castle-party.asm`
